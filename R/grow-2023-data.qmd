...
### Eme confirmed this was a typo and updated it accordingly. Shayna is still using the previously downloaded and corrected google sheet / .csv file for this script.
...
```{r combining oyster abundance summary tables into one table for plotting}
#| echo: false
#| warning: false
#| message: false
...
oyst_abundance_summary_tile <- oyst_abundance_summary_tile %>%
  select(-Plot_Label, -Tile_Tag) %>% 
    mutate(data_type = ifelse(Tile_Type == "Control", "Unseeded",
                            ifelse(Tile_Type == "No-predator", "Ambient Cue",
                                   ifelse(Tile_Type == "Predator", "Predator Cue", "ERROR")))) %>% 
    mutate(tile_or_plot = "Tile")
...
oyst_abundance_summary_fig <- oyst_abundance_summary_fig %>%
  mutate(oyst_facets = ifelse(Date == "8/2/23" & tile_or_plot == "Tile", "b) Initial - tiles",
                         ifelse(Date == "8/2/23" & tile_or_plot == "Plot", "a) Initial - plots",
                                ifelse(Date == "9/6/23" & tile_or_plot == "Tile", "d) 1 Month Post-Deployment - tiles",
                                       ifelse(Date == "9/6/23" & tile_or_plot == "Plot", "c) 1 Month Post-Deployment - plots", "ERROR")))))



oyst_abundance_summary_fig$data_type <- factor(oyst_abundance_summary_fig$data_type, levels = c("Unseeded", "Ambient Cue", "Predator Cue", "Overall Plot"))
...
```{r combining oyster size data summary tables into one table for plotting}
...
# for oyst_size_summary_tile summary table
oyst_size_summary_tile <- oyst_size_summary_tile %>%
  select(-Plot_Label, -Tile_Tag) %>% 
    mutate(data_type = ifelse(Tile_Type == "Control", "Unseeded",
                            ifelse(Tile_Type == "No-predator", "Ambient Cue",
                                   ifelse(Tile_Type == "Predator", "Predator Cue", "ERROR")))) %>% 
...
# for oyst_size_summary_plot summary table
oyst_size_summary_plot <- oyst_size_summary_plot %>% 
  rename(Oyster_Length_mm_Mean = Oyster_Length_mm_Mean_Mean_Mean,
         Oyster_Length_mm_n = Oyster_Length_mm_Mean_Mean_n,
         Oyster_Length_mm_SD = Oyster_Length_mm_Mean_Mean_SD,
         Oyster_Length_mm_SE = Oyster_Length_mm_Mean_Mean_SE)
...
# for oyst_size_summary_plot_type summary table
oyst_size_summary_plot_type <- oyst_size_summary_plot_type %>% 
  rename(Oyster_Length_mm_Mean = Oyster_Length_mm_Mean_Mean_Mean,
         Oyster_Length_mm_n = Oyster_Length_mm_Mean_Mean_n,
         Oyster_Length_mm_SD = Oyster_Length_mm_Mean_Mean_SD,
         Oyster_Length_mm_SE = Oyster_Length_mm_Mean_Mean_SE) %>% 
...
# create summary table from above tables
oyst_size_summary_fig <- bind_rows(oyst_size_summary_tile,
                                   oyst_size_summary_plot,
                                   oyst_size_summary_plot_type)


# add in lack of data for initial unseeded tiles
oyst_size_summary_fig <- oyst_size_summary_fig %>%
  add_row(Date = "8/2/23",
          Plot_Type = "Seagrass+Tiles",
          Tile_Type = "Control",
          data_type = "Unseeded",
...
oyst_size_summary_fig$data_type <- factor(oyst_size_summary_fig$data_type, levels = c("Unseeded", "Ambient Cue", "Predator Cue", "Overall Plot"))
...
oyster_custom_ggplot <- function(summary1, variable_name, plot_type1, plot_type2, y_axis_label, axis_y_limits, y_axis_min, y_axis_max) {
  gg <- ggplot(summary1, aes(x = data_type, y = !!sym(paste0(variable_name, "_Mean")), group = Plot_Type)) +
        facet_wrap(~oyst_facets, nrow = 2, ncol = 2,
                   # labeller = labeller(Date = c("8/2/23" = "a) Initial", "9/6/23" = "b) One Month Post-Deployment")),
               # labeller = labeller(Date = c("8/2/23" = "a) Initial", "9/6/23" = "b) One Month Post-Deployment")),
               scales = "free") +
        geom_point(data = axis_y_limits, aes(x = data_type, y = value, group = Plot_Type), alpha = 0) +
    theme_classic() + 
    geom_linerange(aes(ymin = !!sym(paste0(variable_name, "_Mean")) - !!sym(paste0(variable_name, "_SE")),
                      ymax = !!sym(paste0(variable_name, "_Mean")) + !!sym(paste0(variable_name, "_SE")),
                      group = Plot_Type,
...
## Oyster Abundance Figure

axis_y_limits <- data.frame(data_type = rep(c("Unseeded",
                                          "Ambient Cue",
                                          "Predator Cue",
                                          "Overall Plot"), each = 8),
                            oyst_facets = rep(c("a) Initial - plots",
                                         "b) Initial - tiles",
                                         "c) 1 Month Post-Deployment - plots",
                                         "d) 1 Month Post-Deployment - tiles"), 2),
                            Plot_Type = rep(c("Seagrass+Tiles", "Tiles"), each = 4),
                            value = rep(c(25,25,1.5,1.5),2))

axis_y_limits <- axis_y_limits[-c(26,28,30,32),]
axis_y_limits$data_type <- factor(axis_y_limits$data_type, levels = c("Unseeded", "Ambient Cue", "Predator Cue", "Overall Plot"))


Fig_1_Oyster_Abundance <- oyster_custom_ggplot(oyst_abundance_summary_fig, "Oyster_Abundance", y_axis_label = "Oyster Abundance", axis_y_limits = axis_y_limits, y_axis_min = 0, y_axis_max = 35)

Fig_1_Oyster_Abundance

ggsave("../plots/Fig_1_oyster_abundance_v2.pdf", width = 8.5, height = 6.5, units = "in")


## Oyster Size Figure
Fig_2_Oyster_Size <- oyster_custom_ggplot(oyst_size_summary_fig, "Oyster_Length_mm", y_axis_label = "Oyster Length (mm)", y_axis_min = 8, y_axis_max = 22)

Fig_2_Oyster_Size

ggsave("../plots/Fig_2_oyster_size_v2.pdf", width = 8.5, height = 6.5, units = "in")
...
## Change in Seagrass Metrics

```{r calculating change in seagrass percent cover and canopy height}

## Seagrass Percent Cover Data

### Initial Percent Cover
init_sg_perc_cover_seagrass_tiles <- sg_summary_plot_type %>% 
  filter(Date == "7/31/2023" & Plot_Type == "Seagrass+Tiles") %>% 
  pull(Percent_Cover_Mean_Mean)

init_sg_perc_cover_only_seagrass <- sg_summary_plot_type %>% 
  filter(Date == "7/31/2023" & Plot_Type == "Seagrass") %>% 
  pull(Percent_Cover_Mean_Mean)


### One-month post deployment Percent Cover
month_1_sg_perc_cover_seagrass_tiles <- sg_summary_plot_type %>% 
  filter(Date == "9/6/23" & Plot_Type == "Seagrass+Tiles") %>% 
  pull(Percent_Cover_Mean_Mean)

month_1_sg_perc_cover_only_seagrass <- sg_summary_plot_type %>% 
  filter(Date == "9/6/23" & Plot_Type == "Seagrass") %>% 
  pull(Percent_Cover_Mean_Mean)


init_sg_perc_cover_seagrass_tiles
init_sg_perc_cover_only_seagrass
month_1_sg_perc_cover_seagrass_tiles
month_1_sg_perc_cover_only_seagrass
  

change_sg_perc_cover_seagrass_tiles <- (month_1_sg_perc_cover_seagrass_tiles - init_sg_perc_cover_seagrass_tiles) / init_sg_perc_cover_seagrass_tiles * 100
change_sg_perc_cover_only_seagrass <- (month_1_sg_perc_cover_only_seagrass - init_sg_perc_cover_only_seagrass) / init_sg_perc_cover_only_seagrass * 100


change_sg_perc_cover_seagrass_tiles
change_sg_perc_cover_only_seagrass


## Seagrass Canopy Height Data


### Initial Canopy Height
init_sg_canopy_ht_seagrass_tiles <- sg_summary_plot_type %>% 
  filter(Date == "7/31/2023" & Plot_Type == "Seagrass+Tiles") %>% 
  pull(Canopy_Height_cm_Mean_Mean)

init_sg_canopy_ht_only_seagrass <- sg_summary_plot_type %>% 
  filter(Date == "7/31/2023" & Plot_Type == "Seagrass") %>% 
  pull(Canopy_Height_cm_Mean_Mean)


### One-month post deployment Percent Cover - NA because canopy height was not assessed at this time point
# month_1_sg_canopy_ht_seagrass_tiles <- sg_summary_plot_type %>% 
#   filter(Date == "9/6/23" & Plot_Type == "Seagrass+Tiles") %>% 
#   pull(Canopy_Height_cm_Mean_Mean)
# 
# month_1_sg_canopy_ht_only_seagrass <- sg_summary_plot_type %>% 
#   filter(Date == "9/6/23" & Plot_Type == "Seagrass") %>% 
#   pull(Canopy_Height_cm_Mean_Mean)


### UNCOMMENT BELOW WHEN HAVE MORE CANOPY HEIGHT DATA FOR SEAGRASS
# init_sg_canopy_ht_seagrass_tiles
# init_sg_canopy_ht_only_seagrass
# 
#   
# ## calculations of percent change - update for future data on canopy height
# change_sg_canopy_ht_seagrass_tiles <- (#FILL IN# 
#   - init_sg_canopy_ht_seagrass_tiles) / init_sg_canopy_ht_seagrass_tiles * 100
# change_sg_canopy_ht_only_seagrass <- (#FILL IN# 
#   - init_sg_canopy_ht_only_seagrass) / init_sg_canopy_ht_only_seagrass * 100
# 
# 
# change_sg_canopy_ht_seagrass_tiles
# change_sg_canopy_ht_only_seagrass
...
## Change in Oyster Metrics

```{r calculating change in oyster abundance and length}

## Oyster Abundance

init_oyst_abund_seagrass_tiles <- oyst_abundance_summary_plot_type %>% 
  filter(Date == "8/2/23" & Plot_Type == "Seagrass+Tiles") %>% 
  pull(Oyster_Abundance_Mean)


init_oyst_abund_only_tiles <- oyst_abundance_summary_plot_type %>% 
  filter(Date == "8/2/23" & Plot_Type == "Tiles") %>% 
  pull(Oyster_Abundance_Mean)


month_1_oyst_abund_seagrass_tiles <- oyst_abundance_summary_plot_type %>% 
  filter(Date == "9/6/23" & Plot_Type == "Seagrass+Tiles") %>% 
  pull(Oyster_Abundance_Mean)


month_1_oyst_abund_only_tiles <- oyst_abundance_summary_plot_type %>% 
  filter(Date == "9/6/23" & Plot_Type == "Tiles") %>% 
  pull(Oyster_Abundance_Mean)


init_oyst_abund_seagrass_tiles
init_oyst_abund_only_tiles
month_1_oyst_abund_seagrass_tiles
month_1_oyst_abund_only_tiles
  

change_oyst_abund_seagrass_tiles <- (month_1_oyst_abund_seagrass_tiles - init_oyst_abund_seagrass_tiles) / init_oyst_abund_seagrass_tiles * 100
change_oyst_abund_only_tiles <- (month_1_oyst_abund_only_tiles - init_oyst_abund_only_tiles) / init_oyst_abund_only_tiles * 100


change_oyst_abund_seagrass_tiles
change_oyst_abund_only_tiles


## Oyster Length

init_oyst_length_seagrass_tiles <- oyst_size_summary_plot_type %>% 
  filter(Date == "8/2/23" & Plot_Type == "Seagrass+Tiles") %>% 
  pull(Oyster_Length_mm_Mean)

init_oyst_length_only_tiles <- oyst_size_summary_plot_type %>% 
  filter(Date == "8/2/23" & Plot_Type == "Tiles") %>% 
  pull(Oyster_Length_mm_Mean)

month_1_oyst_length_seagrass_tiles <- oyst_size_summary_plot_type %>% 
  filter(Date == "9/6/23" & Plot_Type == "Seagrass+Tiles") %>% 
  pull(Oyster_Length_mm_Mean)

month_1_oyst_length_only_tiles <- oyst_size_summary_plot_type %>% 
  filter(Date == "9/6/23" & Plot_Type == "Tiles") %>% 
  pull(Oyster_Length_mm_Mean)


init_oyst_length_seagrass_tiles
init_oyst_length_only_tiles
month_1_oyst_length_seagrass_tiles
month_1_oyst_length_only_tiles
  

change_oyst_length_seagrass_tiles <- (month_1_oyst_length_seagrass_tiles - init_oyst_length_seagrass_tiles) / init_oyst_length_seagrass_tiles * 100
change_oyst_length_only_tiles <- (month_1_oyst_length_only_tiles - init_oyst_length_only_tiles) / init_oyst_length_only_tiles * 100


change_oyst_length_seagrass_tiles
change_oyst_length_only_tiles
